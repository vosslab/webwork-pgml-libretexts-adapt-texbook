...
## Overview
- This repo does not build an application; it maintains a LibreTexts-imported textbook in `Textbook`.
- The primary workflow is: edit chapter HTML under `Textbook`, keep content PGML-first, then lint HTML
  for LibreTexts compatibility.
- Supporting tooling lives in `tools` (authoring utilities) and `tests` (local verification
  scripts).
...
- HTML authoring rules ([LibreTexts HTML guide](LIBRETEXTS_HTML_GUIDE.md))
  ...
  - Key dependency: enforced by [html_lint_checker.py](../tools/html_lint_checker.py).
- HTML linting tool ([html_lint_checker.py](../tools/html_lint_checker.py))
  ...
  - Entry point: invoked by [run_html_lint.sh](../tests/run_html_lint.sh).
- Test scripts (`tests`)
  - What they do: run local checks for HTML compatibility and Python linting.
  - Entry points:
    - [run_html_lint.sh](../tests/run_html_lint.sh) runs the HTML lint checker against `Textbook/`.
    - `run_pyflakes.sh` runs `pyflakes` over `*.py` and writes `pyflakes.txt` at repo
      root.
- Source material archives (`Insight-HTML/`, `WebWorK-HTML/`, `Links/`)
  ...
  - Notes: [extract_url_links_from_html_file.py](../tools/extract_url_links_from_html_file.py) and
    [get_insight.py](../tools/get_insight.py) relate to collecting these references.
- Web rendering fetch tool ([get_insight.py](../tools/get_insight.py))
...
- PGML validation pipeline
  - [lint_textbook_problems.py](../tools/lint_textbook_problems.py) orchestrates the full workflow:
...
- Primary authoring flow
  - Edit chapter HTML under `Textbook` (PGML-first writing with minimal PG scaffolding).
...
- Reference ingestion flow (optional)
  ...
  - Run [get_insight.py](../tools/get_insight.py) to render and save simplified HTML snapshots to a chosen output
    directory.
  - Use the saved snapshots as writing input for `Textbook` content (not as imported textbook pages).

## Testing and verification
- HTML linting: [run_html_lint.sh](../tests/run_html_lint.sh) runs
  [html_lint_checker.py](../tools/html_lint_checker.py) against `Textbook/`.
- Python linting: `run_pyflakes.sh` runs `pyflakes` across repo `*.py` and writes
  `pyflakes.txt` (ignored by `.gitignore`).

## Extension points
- New chapters and pages: add `*.html` pages under `Textbook` and update
...
- New authoring rules: update the [LibreTexts HTML guide](LIBRETEXTS_HTML_GUIDE.md) and, when enforcement is needed,
  add checks to [html_lint_checker.py](../tools/html_lint_checker.py).
- New utilities: add single-purpose scripts under `tools` and document the intended workflow in
  `README.md` (and in `docs/` when it is a reusable convention).
- New verification steps: add scripts under `tests` (keep them runnable directly and repo-root aware).
